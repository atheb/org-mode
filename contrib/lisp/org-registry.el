...
(defun org-registry-get-entries (file)
  ...
  (let (bufstr result inside-fstree-block)
    (with-temp-buffer
      ...
      (while (re-search-forward (org-registry-regexp-or-fstree-block org-any-link-re) nil t)
        (let ((match-end-point (match-end 0)))
		 ...
		 (let (point link desc heading-components heading-id)
		   (cond ((looking-at org-bracket-link-regexp)
			  (setq point (match-beginning 0))
			  (setq link (match-string-no-properties 1))
			  (setq desc (or (match-string-no-properties 3) "No description")))
			 ((or (looking-at org-plain-link-re) (looking-at org-angle-link-re))
			  (setq point (match-beginning 0))
			  (setq link (concat (match-string-no-properties 1) ":" (match-string-no-properties 2)))
			  (setq desc link)))
                   (save-excursion 
                     (unless (org-before-first-heading-p)
